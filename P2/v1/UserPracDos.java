...
public class UserPracDos{
	public static void main(String[] args) {
		...
		try{
			File archi = new File("prueba2.txt");	//abrir archivo
			...
			ArrayList<String> okok=new ArrayList<String>();		//array vacío para caminos
			ArrayList<String> okok2=new ArrayList<String>();	//array vacío para manejo de errores
			//diswe.Validar(states, finalstates, initialstate, alphabet, set, initialstate.singleelement(0), okok, 0);
			okok.add(initialstate.singleelement(0));		//inicializa el array de camino con el estado inicial
			diswe.Validar(states, finalstates, initialstate, alphabet, set, initialstate.singleelement(0),0,okok,okok2);	//con esto se valida todo así bien sabroso

			/*
			String prueba="E";
			if (prueba.equals("E")) {
				System.out.println("SIWE");
			}
			*/
